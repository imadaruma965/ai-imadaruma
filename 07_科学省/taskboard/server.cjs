...
// 値が入れば buildSmartRabbitContext が02_経産省/strategy/*.mdより優先して使う。
...
async function basePrompt(date) {
  const [persona, skill, recentLog, today, stateRecord] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "03_内務省", "sontoku.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "sontoku", "SKILL.md"), 10000),
    readOptional(path.join(REPO_ROOT, "03_内務省", "daily_governance", "sontoku_session_log.md"), 14000),
    readOptional(path.join(REPO_ROOT, "03_内務省", "daily_governance", "today.md"), 10000),
    readStateRecord(),
  ]);
...
async function smartRabbitBasePrompt(date, mode) {
  ...
  const [persona, skill, recentLog, today] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "01_首相官邸", "smart_rabbit.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "smart_rabbit", "SKILL.md"), 12000),
    readOptional(path.join(REPO_ROOT, "03_内務省", "daily_governance", "smart_rabbit_session_log.md"), 8000),
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => {
    ...
    if (tailscaleUrl) {
      ...
      console.log("携帯（外出先）: Tailscale未設定 → 07_科学省/taskboard/setup-tailscale.sh を参照");
    }
    console.log(
