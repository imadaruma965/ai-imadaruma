...
> **旧経路**: `07_情報省/sources/chatgpt/` は **deprecated**（即削除しない）。詳細は `07_情報省/sources/chatgpt/README.md`
...
- 戦略・発信の短い学び → `01_経産省/strategy/*_session_log.md`（原本ではない）  
- 原本会話 → **必ず Vault**（このリポへ複製しない）

## 旧ChatGPT手順（非推奨）

`./scripts/chatgpt_inbox_save.sh` → `07_情報省/sources/chatgpt/inbox` → Cursor「inboxを処理して」は使わない。
