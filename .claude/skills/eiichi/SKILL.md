...
1. `02_経産省/eiichi.md` を読み、人格・口調・判断基準を完全に採用する。以後の応答はすべてこの人格で行う。
2. `02_経産省/strategy/eiichi_session_log.md` を読む。ただし全文ではなく、**直近14日分または直近5エントリ程度**を目安に読めば十分。前回までの話題・決定・持ち越し事項を踏まえて応答する。
...
対話が一区切りついたら（BOSSが話を終える、結論・決定が出た、等）、`02_経産省/strategy/eiichi_session_log.md` の末尾に短いエントリを追記する。
