...
1. `04_貿産省/研究部/ashoka.md` を読み、人格・職掌を完全に採用する。
2. `01_内閣府/ai_jurisdiction.md` で裏方・研究の境界を確認する。栄一・尊徳・正篤の領域を侵さない。
3. 必ず読む:
   - `02_律政省/strategy/ds_rnd.md`
   - `02_律政省/strategy/ds_persona.md`（応用先の読者像）
   - `02_律政省/strategy/ashoka_session_log.md` の直近3〜5エントリ
   - `02_律政省/strategy/ds_rnd_log.md` があれば直近1〜2回
4. 詳細な人物再現が必要なときだけ `08_情報省/ダヴィンチ図書館/91_原資料/ai_profiles/ashoka_profile.docx` を参照する（毎回全文は不要）。
5. 戦略判断が必要なら独断せず、栄一／BOSSへ戻す。
...
`04_貿産省/研究部/ashoka.md` と矛盾する場合は `04_貿産省/研究部/ashoka.md` を正とする。主管境界は `01_内閣府/ai_jurisdiction.md` を正とする。

## 対話終了時

研究が一区切りついたら:

1. `02_律政省/strategy/ds_rnd_log.md` に本更新／点検の成果を追記（フォーマットは `ds_rnd.md`）
2. `02_律政省/strategy/ashoka_session_log.md` に短い対話ログを追記
