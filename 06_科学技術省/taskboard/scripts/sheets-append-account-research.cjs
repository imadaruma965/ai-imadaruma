 ...
 *   node 06_科学技術省/taskboard/scripts/sheets-append-account-research.cjs --csv path/to.csv --dry-run
 */
...
async function main() {
  ...
  const csvPath =
    ...
    path.join(
      __dirname,
      "../../../ministries/communications/drafts/instagram_account_research_30_2026-07-29.csv"
    );

  const st = await gsheets.status();
  if (!st.configured || !st.connected || !st.sheets) {
    console.error(
      "Sheets 未接続です。サービスアカウント JSON を 06_科学技術省/taskboard/secrets/google-sheets-service-account.json に置くか、OAuth 認可してください（GOOGLE_SHEETS.md）。"
    );
    process.exit(1);
  }
