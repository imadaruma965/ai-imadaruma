...
// cwd や起動シェルに依存せず、06_科学技術省/taskboard/.env.local を読む（既存envは上書きしない）
loadLocalEnv();
...
const CHAT_LOG_DIR = path.join(REPO_ROOT, "02_厚生労働省", "daily_governance", "chat_logs");
...
// 値が入れば buildSmartRabbitContext が01_経済産業省/strategy/*.mdより優先して使う。
...
async function basePrompt(date) {
  const [persona, skill, recentLog, today, stateRecord] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "02_厚生労働省", "sontoku.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "sontoku", "SKILL.md"), 10000),
    readOptional(path.join(REPO_ROOT, "02_厚生労働省", "daily_governance", "sontoku_session_log.md"), 14000),
    readOptional(path.join(REPO_ROOT, "02_厚生労働省", "daily_governance", "today.md"), 10000),
    readStateRecord(),
  ]);
...
async function smartRabbitBasePrompt(date, mode) {
  ...
  const [persona, skill, recentLog, today] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "00_総理官邸", "smart_rabbit.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "smart_rabbit", "SKILL.md"), 12000),
    readOptional(path.join(REPO_ROOT, "02_厚生労働省", "daily_governance", "smart_rabbit_session_log.md"), 8000),
    readOptional(path.join(REPO_ROOT, "02_厚生労働省", "daily_governance", "today.md"), 8000),
  ]);
...
【日次統治の現況 02_厚生労働省/daily_governance/today.md】
${today || "(未記入)"}`;
}
...
async function appendChatLog({ date, message, event, response, agentId, runId }) {
  ...
  const header = exists
    ? ""
    : `# AI尊徳・キングダムOS 対話ログ — ${date}\n\n> Cursor SDK agent: ${agentId}\n> キングダムOSから自動記録。要約は \`02_厚生労働省/daily_governance/sontoku_session_log.md\` に残す。\n\n`;
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => {
    ...
    if (tailscaleUrl) {
      ...
      console.log("携帯（外出先）: Tailscale未設定 → 06_科学技術省/taskboard/setup-tailscale.sh を参照");
    }
    console.log(
    ...
    console.log(
      hasCursorApiKey()
        ...
        : "AI尊徳 / スマートラビット: 未接続（06_科学技術省/taskboard/.env.local に CURSOR_API_KEY を設定してください）"
    );
  });
}
