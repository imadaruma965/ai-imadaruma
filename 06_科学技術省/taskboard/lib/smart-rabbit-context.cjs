...
// 01_経済産業省/strategy/*.md はKingdom OSの現行の正式ドキュメント(Knowledgeとは別の業務stateの一次情報)。
...
function buildGoalsSection(now) {
  return safe(() => {
    const doc = loadDoc("01_経済産業省/strategy/roadmap_500k.md");
...
function buildYesterdayReviewSection(now) {
  return safe(() => {
    const doc = loadDoc("02_厚生労働省/daily_governance/night_review.md");
...
function buildInstagramProfileSection(state, now) {
  return safe(() => {
    ...
    const facts = [];
    let updatedAt = null;
    let source = "01_経済産業省/strategy/instagram_growth_plan.md";
    if (business && (business.brandName || business.tagline || (business.pillars || []).length)) {
      ...
      const doc = loadDoc("01_経済産業省/strategy/instagram_growth_plan.md");
...
function buildProductsSection(state, now) {
  return safe(() => {
    ...
    let facts = [];
    let updatedAt = null;
    let source = "01_経済産業省/strategy/offer_review.md";
    if (business.length) {
      facts = business
        .slice(0, 4)
      ...
      const doc = loadDoc("01_経済産業省/strategy/offer_review.md");
