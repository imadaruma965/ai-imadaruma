...
仕様: [`01_経済産業省/strategy/taskboard_mvp_spec.md`](../../01_経済産業省/strategy/taskboard_mvp_spec.md)
...
1. Finder で `06_科学技術省/taskboard/start.command` を開く  
...
```bash
cd /Users/imadatadahito/Desktop/ai-imadaruma/06_科学技術省/taskboard
./start.sh
```
...
定型応答ではなく、`02_厚生労働省/sontoku.md`、直近の尊徳ログ、今日の任務を踏まえて会話する。

1. [Cursor Dashboard → Integrations](https://cursor.com/dashboard/integrations) で APIキーを作る
2. `06_科学技術省/taskboard/.env.local` を作り、次の1行を保存する
...
```bash
chmod 600 06_科学技術省/taskboard/.env.local
```

4. `start.command` または `./start.sh` で起動する

サーバー本体（`server.cjs`）も起動時に `06_科学技術省/taskboard/.env.local` を自動読込する（cwd やシェルの `source` に依存しない）。既存の環境変数は上書きしない。尊徳とスマートラビットは同じ判定（`hasCursorApiKey`）を使う。

`.env.local` はGitの対象外であり、APIキーはブラウザへ送られない。
初回起動時は Cursor SDK を自動インストールする。使用モデルは既定で `auto`。

対話は日付ごとのCursorエージェントとして継続し、生ログを
`02_厚生労働省/daily_governance/chat_logs/sontoku-YYYY-MM-DD.md` に自動保存する。
...
1. `06_科学技術省/taskboard/.env.local` に長いトークンを設定する
...
データは **Macの `06_科学技術省/taskboard/data/state.json`** が正本。通常はJSON不要。
...
6. 週1: **JSON保存** でバックアップ（`06_科学技術省/taskboard/data/` 推奨）
...
セットアップ: `06_科学技術省/taskboard/GOOGLE_CALENDAR.md`  
...
| AI尊徳が「APIキー未設定」 | `06_科学技術省/taskboard/.env.local` を作成し、キングダムOSを再起動 |
| AI尊徳が「接続エラー」 | 起動中のターミナルを確認。APIキーとCursor利用権限も確認 |
| 携帯で開けない | Macと携帯が同じWi-Fiか確認。`./start.sh` 起動時のLAN URLを使う |
| 携帯で開けるが「認証トークンが正しくありません」 | `06_科学技術省/taskboard/.env.local` に `TASKBOARD_TOKEN` を設定してキングダムOSを再起動（LANアクセスには必須） |
