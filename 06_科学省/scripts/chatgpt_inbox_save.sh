...
#   詳細: 07_情報省/sources/chatgpt/README.md / 02_内務省/daily_governance/chatgpt_capture.md
# ChatGPTのコピー内容を 07_情報省/sources/chatgpt/inbox/ に時刻付きで保存する（非推奨）
set -euo pipefail
echo "警告: chatgpt_inbox_save.sh は deprecated。正本は imada-knowledge（aistock）です。" >&2

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
INBOX="$ROOT/07_情報省/sources/chatgpt/inbox"
TAG="${1:-chat}"
