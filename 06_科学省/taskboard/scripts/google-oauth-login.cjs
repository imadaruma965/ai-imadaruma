 ...
 * 使い方: node 06_科学省/taskboard/scripts/google-oauth-login.cjs
 *
 * 前提: 06_科学省/taskboard/.env.local に GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET
...
async function main() {
  if (!oauth.configured()) {
    console.error(`
GOOGLE OAuth 未設定です。次を 06_科学省/taskboard/.env.local に追加してください:
...
詳細: 06_科学省/taskboard/GOOGLE_SHEETS.md
`);
    process.exit(1);
  }
