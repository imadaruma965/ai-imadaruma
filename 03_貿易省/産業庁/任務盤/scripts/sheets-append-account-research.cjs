 ...
 *   node 03_貿易省/産業庁/任務盤/scripts/sheets-append-account-research.cjs --csv path/to.csv --dry-run
 */
...
async function main() {
  ...
  const csvPath =
    ...
    path.join(
      __dirname,
      "../../../05_情報省/ダヴィンチ図書館/02_国家事業/Instagram/01_リサーチ原本/instagram_account_research_30_2026-07-29.csv"
    );

  const st = await gsheets.status();
  if (!st.configured || !st.connected || !st.sheets) {
    console.error(
      "Sheets 未接続です。サービスアカウント JSON を 03_貿易省/産業庁/任務盤/secrets/google-sheets-service-account.json に置くか、OAuth 認可してください（GOOGLE_SHEETS.md）。"
    );
    process.exit(1);
  }
