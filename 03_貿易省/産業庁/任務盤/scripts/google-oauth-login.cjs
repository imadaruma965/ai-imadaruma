 ...
 * 使い方: node 03_貿易省/産業庁/任務盤/scripts/google-oauth-login.cjs
 *
 * 前提: 03_貿易省/産業庁/任務盤/.env.local に GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET
...
async function main() {
  if (!oauth.configured()) {
    console.error(`
GOOGLE OAuth 未設定です。次を 03_貿易省/産業庁/任務盤/.env.local に追加してください:
...
詳細: 03_貿易省/産業庁/任務盤/GOOGLE_SHEETS.md
`);
    process.exit(1);
  }
