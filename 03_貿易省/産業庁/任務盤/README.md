...
仕様: [`00_律令府/戦略/装置仕様/任務盤MVP仕様.md`](../../../00_律令府/戦略/装置仕様/任務盤MVP仕様.md)
...
1. Finder で `03_貿易省/産業庁/任務盤/start.command` を開く  
...
```bash
cd /Users/imadatadahito/Desktop/ai-imadaruma/03_貿易省/産業庁/任務盤
./start.sh
```
...
定型応答ではなく、`02_修身省/人格正本/尊徳.md`、直近の尊徳ログ、今日の任務を踏まえて会話する。

1. [Cursor Dashboard → Integrations](https://cursor.com/dashboard/integrations) で APIキーを作る
2. `03_貿易省/産業庁/任務盤/.env.local` を作り、次の1行を保存する
...
```bash
chmod 600 03_貿易省/産業庁/任務盤/.env.local
```

4. `start.command` または `./start.sh` で起動する

サーバー本体（`server.cjs`）も起動時に `03_貿易省/産業庁/任務盤/.env.local` を自動読込する（cwd やシェルの `source` に依存しない）。既存の環境変数は上書きしない。尊徳と賢いうさぎは同じ判定（`hasCursorApiKey`）を使う。

`.env.local` はGitの対象外であり、APIキーはブラウザへ送られない。
初回起動時は Cursor SDK を自動インストールする。使用モデルは既定で `auto`。

対話は日付ごとのCursorエージェントとして継続し、生ログを
`02_修身省/日次運用/対話ログ/sontoku-YYYY-MM-DD.md` に自動保存する。
...
各AIは対応する人格正本（例: `04_理財省/ルカ.md`）とSkill、`01_内閣府/AI主管定義.md` の主管境界を踏まえて応答し、自分の主管を超える相談は担当する省への申し送りとして案内する（越権判断はしない）。

接続方式・トークン・Cursor APIキーは上記「AI尊徳を接続する」と共通（追加設定不要）。
ログは `02_修身省/日次運用/対話ログ/cabinet-<メンバーID>-YYYY-MM-DD.md`（省庁の部屋）／`sontoku-YYYY-MM-DD.md`（内務省・今日）／`smart-rabbit-YYYY-MM-DD.md`（首相官邸・相談）に自動保存される。
...
1. `03_貿易省/産業庁/任務盤/.env.local` に長いトークンを設定する
...
データは **Macの `03_貿易省/産業庁/任務盤/data/state.json`** が正本。通常はJSON不要。
...
6. 週1: **JSON保存** でバックアップ（`03_貿易省/産業庁/任務盤/data/` 推奨）
...
セットアップ: `03_貿易省/産業庁/任務盤/GOOGLE_CALENDAR.md`  
...
| AI尊徳が「APIキー未設定」 | `03_貿易省/産業庁/任務盤/.env.local` を作成し、キングダムOSを再起動 |
...
| 携帯で開けるが「認証トークンが正しくありません」 | `03_貿易省/産業庁/任務盤/.env.local` に `TASKBOARD_TOKEN` を設定してキングダムOSを再起動（LANアクセスには必須） |
