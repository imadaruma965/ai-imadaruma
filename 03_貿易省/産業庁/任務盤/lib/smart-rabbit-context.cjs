...
// 00_律令府/戦略/*.md はKingdom OSの現行の正式ドキュメント(Knowledgeとは別の業務stateの一次情報)。
...
function buildGoalsSection(now) {
  return safe(() => {
    const doc = loadDoc("00_律令府/戦略/国家目標/50万円ロードマップ.md");
...
function buildYesterdayReviewSection(now) {
  return safe(() => {
    const doc = loadDoc("02_修身省/日次運用/night_review.md");
...
function buildInstagramProfileSection(state, now) {
  return safe(() => {
    ...
    const facts = [];
    let updatedAt = null;
    let source = "00_律令府/戦略/発信/Instagram成長計画.md";
    if (business && (business.brandName || business.tagline || (business.pillars || []).length)) {
      ...
      const doc = loadDoc("00_律令府/戦略/発信/Instagram成長計画.md");
...
function buildProductsSection(state, now) {
  return safe(() => {
    ...
    let facts = [];
    let updatedAt = null;
    let source = "00_律令府/戦略/商品と営業/商品オファー見直し.md";
    if (business.length) {
      facts = business
        .slice(0, 4)
      ...
      const doc = loadDoc("00_律令府/戦略/商品と営業/商品オファー見直し.md");
