...
// cwd や起動シェルに依存せず、03_貿易省/産業庁/任務盤/.env.local を読む（既存envは上書きしない）
loadLocalEnv();
...
const CHAT_LOG_DIR = path.join(REPO_ROOT, "02_修身省", "日次運用", "対話ログ");
...
// 値が入れば buildSmartRabbitContext が00_律令府/戦略/*.mdより優先して使う。
...
async function basePrompt(date) {
  const [persona, skill, recentLog, today, stateRecord] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "02_修身省", "人格正本", "尊徳.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "sontoku", "SKILL.md"), 10000),
    readOptional(path.join(REPO_ROOT, "02_修身省", "日次運用", "対話ログ", "尊徳.md"), 14000),
    readOptional(path.join(REPO_ROOT, "02_修身省", "日次運用", "今日.md"), 10000),
    readStateRecord(),
  ]);
...
async function smartRabbitBasePrompt(date, mode) {
  ...
  const [persona, skill, recentLog, today] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "01_内閣府", "賢いうさぎ.md"), 20000),
    readOptional(path.join(REPO_ROOT, ".claude", "skills", "smart_rabbit", "SKILL.md"), 12000),
    readOptional(path.join(REPO_ROOT, "02_修身省", "日次運用", "対話ログ", "賢いうさぎ.md"), 8000),
    readOptional(path.join(REPO_ROOT, "02_修身省", "日次運用", "今日.md"), 8000),
  ]);
...
async function cabinetBasePrompt(member, date) {
  const [jurisdiction, persona, skill, recentLog] = await Promise.all([
    readOptional(path.join(REPO_ROOT, "01_内閣府", "AI主管定義.md"), 24000),
...
async function appendChatLog({ date, message, event, response, agentId, runId }) {
  ...
  const header = exists
    ? ""
    : `# AI尊徳・キングダムOS 対話ログ — ${date}\n\n> Cursor SDK agent: ${agentId}\n> キングダムOSから自動記録。要約は \`02_修身省/日次運用/sontoku_session_log.md\` に残す。\n\n`;
    ...
    if (req.method === "GET" && url.pathname === "/api/account-research") {
      try {
        ...
        json(res, 200, {
          ...
          vaultNote: "05_情報省/ダヴィンチ図書館/02_国家事業/Instagram/README.md",
          csvPath: "05_情報省/ダヴィンチ図書館/02_国家事業/Instagram/01_リサーチ原本/instagram_account_research_30_2026-07-29.csv",
          count: rows.length,
          rows,
        });
      } catch (error) {
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => {
    ...
    if (tailscaleUrl) {
      ...
      console.log("携帯（外出先）: Tailscale未設定 → 03_貿易省/産業庁/任務盤/setup-tailscale.sh を参照");
    }
    console.log(
    ...
    console.log(
      hasCursorApiKey()
        ...
        : "AI尊徳 / 賢いうさぎ: 未接続（03_貿易省/産業庁/任務盤/.env.local に CURSOR_API_KEY を設定してください）"
    );
  });
}
