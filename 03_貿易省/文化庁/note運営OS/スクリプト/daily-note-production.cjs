...
const RADAR = path.join(ROOT, "03_貿易省/産業庁/商品開発/note伸び記事レーダー/note-radar.cjs");
...
const PROMPT_FILE = path.join(OS_DIR, "プロンプト/daily-draft.md");
...
function run(command, args, options = {}) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`${command} が終了コード${result.status}で失敗しました${detail ? `\n${detail}` : ""}`);
  }
