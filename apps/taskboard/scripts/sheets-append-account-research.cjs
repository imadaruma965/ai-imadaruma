 ...
 *   node apps/taskboard/scripts/sheets-append-account-research.cjs --csv path/to.csv --dry-run
 */
...
async function main() {
  ...
  if (!st.configured || !st.connected || !st.sheets) {
    console.error(
      "Sheets 未接続です。サービスアカウント JSON を apps/taskboard/secrets/google-sheets-service-account.json に置くか、OAuth 認可してください（GOOGLE_SHEETS.md）。"
    );
    process.exit(1);
  }
