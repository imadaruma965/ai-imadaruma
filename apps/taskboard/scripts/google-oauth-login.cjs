 ...
 * 使い方: node apps/taskboard/scripts/google-oauth-login.cjs
 *
 * 前提: apps/taskboard/.env.local に GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET
...
async function main() {
  if (!oauth.configured()) {
    console.error(`
GOOGLE OAuth 未設定です。次を apps/taskboard/.env.local に追加してください:
...
詳細: apps/taskboard/GOOGLE_SHEETS.md
`);
    process.exit(1);
  }
