...
// strategy/*.md はKingdom OSの現行の正式ドキュメント(Knowledgeとは別の業務stateの一次情報)。
...
function buildGoalsSection(now) {
  return safe(() => {
    const doc = loadDoc("strategy/roadmap_500k.md");
...
function buildYesterdayReviewSection(now) {
  return safe(() => {
    const doc = loadDoc("daily_governance/night_review.md");
...
function buildInstagramProfileSection(state, now) {
  return safe(() => {
    ...
    const facts = [];
    let updatedAt = null;
    let source = "strategy/instagram_growth_plan.md";
    if (business && (business.brandName || business.tagline || (business.pillars || []).length)) {
      ...
      const doc = loadDoc("strategy/instagram_growth_plan.md");
...
function buildProductsSection(state, now) {
  return safe(() => {
    ...
    let facts = [];
    let updatedAt = null;
    let source = "strategy/offer_review.md";
    if (business.length) {
      facts = business
        .slice(0, 4)
      ...
      const doc = loadDoc("strategy/offer_review.md");
