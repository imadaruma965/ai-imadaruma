...
```text
apps/taskboard/secrets/google-sheets-service-account.json
```

（`apps/taskboard/secrets/` は Git 対象外）
...
Calendar 連携と同じ OAuth でも可。`.env.local` に `GOOGLE_CLIENT_ID` / `SECRET` を入れ、`node apps/taskboard/scripts/google-oauth-login.cjs` で認可。詳細は旧手順のまま `GOOGLE_CALENDAR.md` も参照。
...
```bash
# 重複チェックのみ
node apps/taskboard/scripts/sheets-append-account-research.cjs --dry-run

# 本番追記（アカウントリサーチタブ、既存ハンドルはスキップ）
node apps/taskboard/scripts/sheets-append-account-research.cjs
