...
`04_貿易省/産業庁/taskboard/.env.local`（既存の CURSOR_API_KEY の下など）:
...
```bash
# launchd 利用中なら
cd 04_貿易省/産業庁/taskboard && ./install-autostart.sh
# または ./start.sh
```

キングダムOS → **今日** → **Googleに接続** → ブラウザで許可。

トークンは `04_貿易省/産業庁/taskboard/data/google-token.json` に保存（旧 `gcal-token.json` も読込可。Git対象外）。
