...
サーバー本体（`server.cjs`）も起動時に `04_貿易省/産業庁/taskboard/.env.local` を自動読込する（cwd やシェルの `source` に依存しない）。既存の環境変数は上書きしない。尊徳とスマートラビットは同じ判定（`hasCursorApiKey`）を使う。
...
| タブ（部屋） | 中身 | 応対AI |
|------|------|--------|
| **首相官邸**（既定の着地点） | 全体／一覧／新規／相談 をサブタブで切替 | スマートラビット（相談サブタブ） |
| **経産省** | 今月の目標・KPIボード | 栄一 |
