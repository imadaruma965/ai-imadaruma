...
> **正本**: `05_情報省/ダヴィンチ図書館/05_AI対話/`（互換: `~/imada-knowledge` / `~/Documents/imada-knowledge`）  
> **設計**: Vault `Knowledge_Gateway設計.md`（KG-0）／本リポ Smart Rabbit は MCP 検索のみ（KG-1）  
> **旧経路**: `05_情報省/ダヴィンチ図書館/91_原資料/chatgpt/` は **deprecated**（即削除しない）。詳細は `05_情報省/ダヴィンチ図書館/91_原資料/chatgpt/README.md`
...
- 戦略・発信の短い学び → `00_律令府/戦略/対話ログ/`（原本ではない）  
- 再利用する決定・Skill改善 → 学習候補 → `agent_growth_board.md` → 承認後に Vault／Skill（`05_情報省/エージェント成長OS.md`）  
- 原本会話 → **必ず Vault**（このリポへ複製しない）


## 旧ChatGPT手順（非推奨）

`./scripts/chatgpt_inbox_save.sh` → `05_情報省/ダヴィンチ図書館/91_原資料/chatgpt/inbox` → Cursor「inboxを処理して」は使わない。
