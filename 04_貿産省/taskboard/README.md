...
仕様: [`02_律政省/strategy/taskboard_mvp_spec.md`](../../02_律政省/strategy/taskboard_mvp_spec.md)
...
1. Finder で `04_貿産省/taskboard/start.command` を開く  
...
```bash
cd /Users/imadatadahito/Desktop/ai-imadaruma/04_貿産省/taskboard
./start.sh
```
...
2. `04_貿産省/taskboard/.env.local` を作り、次の1行を保存する
...
```bash
chmod 600 04_貿産省/taskboard/.env.local
```

4. `start.command` または `./start.sh` で起動する

サーバー本体（`server.cjs`）も起動時に `04_貿産省/taskboard/.env.local` を自動読込する（cwd やシェルの `source` に依存しない）。既存の環境変数は上書きしない。尊徳とスマートラビットは同じ判定（`hasCursorApiKey`）を使う。
...
1. `04_貿産省/taskboard/.env.local` に長いトークンを設定する
...
データは **Macの `04_貿産省/taskboard/data/state.json`** が正本。通常はJSON不要。
...
6. 週1: **JSON保存** でバックアップ（`04_貿産省/taskboard/data/` 推奨）
...
セットアップ: `04_貿産省/taskboard/GOOGLE_CALENDAR.md`  
...
| AI尊徳が「APIキー未設定」 | `04_貿産省/taskboard/.env.local` を作成し、キングダムOSを再起動 |
...
| 携帯で開けるが「認証トークンが正しくありません」 | `04_貿産省/taskboard/.env.local` に `TASKBOARD_TOKEN` を設定してキングダムOSを再起動（LANアクセスには必須） |
